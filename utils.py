...
def create_train_log_dir(base_dir="train_log"):
    """
    Create a date-based directory for training plots.

    Example:
        train_log/2026-08-12/
    """
...
def plot_training_metrics(train_csv, val_csv, output_dir):
    """
    Plot training and validation metrics from the CSV files
    generated by write_losses().
    """
    ...
    # These are the metrics we care about most for HiDDeN.
    metrics = [
        "loss",
        "encoder_mse",
        "dec_mse",
    ...
    for metric in metrics:
        if metric not in train_df.columns or metric not in val_df.columns:
            continue
